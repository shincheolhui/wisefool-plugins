...
Makes Claude Code work like a disciplined teammate — triage, isolated worktree, tracked issue, verified merge, every session. — Claude Code 를 규율 있는 팀 동료처럼 — 질문엔 조사만, 착수엔 격리 워크트리→이슈 추적→검증된 머지를 한 절차로.
...
> **절차의 정본은 [SKILL.md](skills/work-lifecycle/SKILL.md) 입니다.** 사람이 읽을 수 있는 마크다운이면서 모델이 따르는 것 *그 자체*라, 명세와 동작이 같은 문서입니다. 이 README 는 절차를 중복 서술하지 않습니다 — 규칙이 궁금하면 SKILL.md 가 정답입니다.
